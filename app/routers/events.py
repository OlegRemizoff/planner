...
from app.dao.planner_dao import EventDAO, UsersDAO
...
@router.post("/new")
async def create_event(data: SEvent, user: Users = Depends(get_current_user)) -> dict:
    user_id = user.id
    raw_text = data.description # забираем текст из JSON-формы и передаем его в check_text
    ...
    data.user_id = user_id                  # меняем данные формы на id текущего пользователя
    data.description = str(correxted_text)  # меняем данные формы на корректные
